...
int HacerConsulta (int identificador, char datos [200], char respuesta [512])
{
	MYSQL* conn;
	int err;
	...
	if (identificador !=0)
	{
		p = strtok( datos, "/");				// Ya tenemos el nombre
		strcpy(nombre,p);
...
void AtenderCliente (void *socket)
{
	...
	while (terminar == 0)
	{
		...
		while (p != NULL)
		{
			strcpy(datos,"%s%s/", datos, p);
			p = strtok( NULL, "/");
		}
		
		terminar = HacerConsulta (codigo, datos, respuesta);
...
int main(int argc, char *argv[]) {
	...
	for (i=0;i<25;i++){
		...
		pthread_create(&trhead,NULL,AtenderCliente,&sockets[i]);
